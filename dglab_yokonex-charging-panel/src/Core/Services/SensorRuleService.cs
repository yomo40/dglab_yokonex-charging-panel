...
public class SensorRuleService : IDisposable
{
    ...
    public void Start()
    {
        ...
        _gameEventSubscription = EventBus.Instance.Subscribe<GameEvent>(OnGameEvent);
    ...
    private void OnGameEvent(GameEvent evt)
    {
        ...
        else if (evt.Type == GameEventType.PressureChanged)
        {
            var pressure = ResolveSensorValue(evt, "pressure", evt.NewValue);
            ProcessSensorEvent(evt.TargetDeviceId ?? "", SensorType.Pressure, pressure);
        }
        else if (evt.Type == GameEventType.ExternalVoltageChanged)
        {
            var voltage = ResolveSensorValue(evt, "voltage", evt.NewValue);
            ProcessSensorEvent(evt.TargetDeviceId ?? "", SensorType.ExternalVoltage, voltage);
        }
    }
    ...
    private static double ResolveSensorValue(GameEvent evt, string key, double fallback)
    {
