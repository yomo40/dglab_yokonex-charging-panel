...
namespace ChargingPanel.Core.Devices.Yokonex
{
    ...
    public class YokonexToyBluetoothAdapter : IDevice, IYokonexToyDevice
    {
        ...
        private readonly YokonexDeviceType _deviceType;
        ...
        // IYokonexDevice 属性
        public YokonexDeviceType YokonexType => _deviceType;

        public YokonexToyBluetoothAdapter(
            IBluetoothTransport transport,
            YokonexDeviceType deviceType = YokonexDeviceType.Vibrator,
            YokonexProtocolGeneration generation = YokonexProtocolGeneration.ToyV1_1,
            string? id = null,
            string? name = null)
        {
            ...
            _deviceType = deviceType is YokonexDeviceType.Cup ? YokonexDeviceType.Cup : YokonexDeviceType.Vibrator;
            ProtocolGeneration = generation == YokonexProtocolGeneration.Auto ? YokonexProtocolGeneration.ToyV1_1 : generation;
            Id = id ?? $"yc_toy_{Guid.NewGuid():N}".Substring(0, 20);
            Name = name ?? (_deviceType == YokonexDeviceType.Cup ? "役次元飞机杯" : "役次元跳蛋");
        }
        ...
        public async Task SetStrengthAsync(Channel channel, int value, StrengthMode mode = StrengthMode.Set)
        {
            ...
            int targetC = _motorC.PowerLevel * 5;
            ...
            switch (mode)
            {
                case StrengthMode.Increase:
                    ...
                    if (channel == Channel.AB)
                        targetC = Math.Clamp(targetC + value, 0, 100);
                    break;
                ...
                default:
                    ...
                    if (channel == Channel.AB)
                        targetC = Math.Clamp(value, 0, 100);
                    break;
            }
            ...
            int powerC = (int)Math.Round(targetC / 5.0);
