...
/// 
/// 协议流程:
/// 1. 用户从役次元 APP 获取连接码 (connect_code)
...
/// </summary>
public class YokonexIMAdapter : IDevice, IYokonexCommandDevice, IDisposable
{
    ...
    public YokonexDeviceType YokonexType => DeviceSubType;
...
#pragma warning restore CS0067
    ...
    // 认证信息
    private string? _connectCode;   // 连接码: "uid token"
    private string? _uid;           // 用户 ID (不带 game_ 前缀)
    private string? _token;         // 游戏鉴权令牌
    private string? _targetUserId;  // 消息目标用户 ID (不带 game_ 前缀)
    ...
    private DateTime? _lastReceiveAtUtc;
    ...
    private const int ReconnectBaseDelayMs = 1500;
    private const int ReconnectMaxDelayMs = 30000;

    // 队列与发送稳定性
    private const int MaxSendQueueSize = 256;
    private const int SendRetryAttempts = 3;
    private const int SendRetryBaseDelayMs = 200;
    private const int ReadyWaitTimeoutMs = 5000;

    // 鉴权与连接超时
    private const int GameSignRetryAttempts = 3;
    ...
    /// 连接到役次元 IM
    /// </summary>
    /// <param name="config">连接配置，需要包含 connect_code（兼容旧版 uid/token）</param>
    ...
    public async Task ConnectAsync(ConnectionConfig config, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            StopReconnectTimer();
            var previousCts = Interlocked.Exchange(ref _cts, CancellationTokenSource.CreateLinkedTokenSource(cancellationToken));
            previousCts?.Cancel();
            previousCts?.Dispose();

            await CleanupTransportAsync(logoutNative: true);
            ClearPendingSendQueue(new OperationCanceledException("会话已切换，未发送消息已取消。"));
            ...
            // 从 connect_code 解析凭据；兼容旧版 uid/token 回退
            if (!TryResolveCredentials(config, out var rawUid, out var rawToken, out var normalizedConnectCode))
            {
                throw new ArgumentException("connect_code 无效，或缺少可回退的 uid/token");
            }

            _connectCode = normalizedConnectCode;
            _uid = NormalizeUid(rawUid);
            _token = rawToken.Trim();
            _targetUserId = NormalizeUid(config.TargetUserId) ?? _uid;

            if (string.IsNullOrWhiteSpace(_uid) || string.IsNullOrWhiteSpace(_token))
            {
                throw new InvalidOperationException("IM 鉴权凭据解析失败（uid/token 为空）");
            }

            UpdateStatus(DeviceStatus.Connecting);
            Logger.Information("正在连接役次元 IM: uid={Uid}, target={Target}, 模式={Mode}", _uid, _targetUserId, ConnectionMode);

            var ct = _cts?.Token ?? cancellationToken;

            // 1. 获取 IM 签名（带重试）
            await GetGameSignWithRetryAsync(_connectCode, $"game_{_uid}", _token, ct);

            // 2. 根据连接模式建立会话
            if (ConnectionMode == IMConnectionMode.NativeSDK)
            {
                await ConnectNativeSDKAsync(ct);
            }
            else
            {
            ...
            // 3. 启动发送队列与保活
            EnsureSendWorkerStarted(ct);
            if (ConnectionMode == IMConnectionMode.WebSocket)
            {
                StartHeartbeat(ct);
            }
    ...
    private async Task ConnectNativeSDKAsync(CancellationToken ct)
    {
        ...
        _nativeService.NetworkStatusChanged += OnNativeNetworkStatusChanged;
        _nativeService.UserSigExpired += OnNativeUserSigExpired;
        ...
        if (!loginSuccess)
        {
            ...
            await DetachNativeServiceAsync(logout: true);
            ConnectionMode = IMConnectionMode.WebSocket;
            await ConnectWebSocketModeAsync(ct);
            return;
        }
                    ...
                    if (elemType == 0 && elem.TryGetProperty("text_elem_content", out var textContent))
                    {
                        ...
                        if (!string.IsNullOrEmpty(text))
                        {
                            try
                            {
                                ...
                                {
                                    parsed.From = from;
                                    _lastReceiveAtUtc = DateTime.UtcNow;
    ...
    private void OnNativeNetworkStatusChanged(object? sender, TencentIMNative.TIMNetworkStatus status)
    {
        Logger.Information("原生 IM 网络状态变化: {Status}", status);
        if (_manualDisconnect)
        {
            return;
        }

        if (status is TencentIMNative.TIMNetworkStatus.kTIMDisconnected or TencentIMNative.TIMNetworkStatus.kTIMConnectFailed)
        {
            _isLoggedIn = false;
            _isReady = false;
            UpdateStatus(DeviceStatus.Disconnected);
    ...
    private void OnNativeUserSigExpired(object? sender, EventArgs e)
    {
        Logger.Warning("原生 IM userSig 已过期，准备刷新签名并重连");
        _isLoggedIn = false;
        _isReady = false;
        UpdateStatus(DeviceStatus.Disconnected);
        StartReconnectTimer();
    ...
    public async Task DisconnectAsync()
    {
        ...
        try
        {
            ...
            await CleanupTransportAsync(logoutNative: true);
            ClearPendingSendQueue(new OperationCanceledException("设备已断开连接，未发送消息已取消。"));
    ...
    {
        // game_cmd 格式: { code: "game_cmd", id: "事件ID", token: "..." }
        var message = new
        {
            code = "game_cmd",
            id = eventId,
    ...
    private async Task GetGameSignWithRetryAsync(string? connectCode, string uid, string token, CancellationToken ct)
    {
        Exception? lastError = null;
        for (var attempt = 1; attempt <= GameSignRetryAttempts; attempt++)
        {
            try
            {
                await GetGameSignAsync(connectCode, uid, token, ct);
                return;
            }
            catch (Exception ex) when (attempt < GameSignRetryAttempts)
            {
                lastError = ex;
                var delay = SendRetryBaseDelayMs * attempt;
                Logger.Warning(ex, "获取游戏签名失败（第 {Attempt}/{Max} 次），{Delay}ms 后重试", attempt, GameSignRetryAttempts, delay);
                await Task.Delay(delay, ct);
            }
        }

        throw new Exception("获取游戏签名失败，已达到最大重试次数", lastError);
    }

    /// <summary>
    /// 获取游戏签名
    /// </summary>
    private async Task GetGameSignAsync(string? connectCode, string uid, string token, CancellationToken ct)
    {
        object requestPayload = !string.IsNullOrWhiteSpace(connectCode)
            ? new { connect_code = connectCode }
            : new { uid, token };
        var requestBody = JsonSerializer.Serialize(requestPayload);
        var content = new StringContent(requestBody, Encoding.UTF8, "application/json");

        Logger.Debug("请求游戏签名: hasConnectCode={HasConnectCode}, uid={Uid}", !string.IsNullOrWhiteSpace(connectCode), uid);

        var response = await _httpClient.PostAsync($"{API_BASE}/user/game_sign", content, ct);
    ...
    private async Task ConnectWebSocketAsync(CancellationToken ct)
    {
        ...
        connectTimeoutCts.CancelAfter(TimeSpan.FromSeconds(ConnectTimeoutSeconds));
        await _ws.ConnectAsync(new Uri(wsUrl), connectTimeoutCts.Token);
    ...
    private async Task LoginAsync(CancellationToken ct)
    {
        ...
        while (!_isLoggedIn && string.IsNullOrWhiteSpace(_lastLoginError) && DateTime.UtcNow < timeout && !ct.IsCancellationRequested)
        {
            await Task.Delay(100, ct);
        }

        if (!string.IsNullOrWhiteSpace(_lastLoginError))
        {
            throw new InvalidOperationException($"IM 登录失败: {_lastLoginError}");
        }
    ...
    private async Task ReceiveLoopAsync(CancellationToken ct)
    {
        ...
        {
            if (!ct.IsCancellationRequested && !_manualDisconnect)
            {
                UpdateStatus(DeviceStatus.Disconnected);
                // 触发自动重连
                StartReconnectTimer();
    ...
    private Task EnqueueMessageAsync(object payload)
    {
        ...
        {
            throw new InvalidOperationException($"IM 发送队列已满（上限 {MaxSendQueueSize}），请稍后重试。");
        }

        var queued = new QueuedMessage(payload);
        _sendQueue.Enqueue(queued);
        _sendQueueSignal.Release();
        return queued.Completion.Task;
    }

    private void EnsureSendWorkerStarted(CancellationToken ct)
    {
        ...
        _sendTask = Task.Run(() => ProcessSendQueueAsync(ct), ct);
    }
    ...
    private async Task ProcessSendQueueAsync(CancellationToken ct)
    {
        try
        {
            ...
            {
                await _sendQueueSignal.WaitAsync(ct);

                while (_sendQueue.TryDequeue(out var queued))
                {
                    try
                    {
                        await SendWithRetryAsync(queued.Payload, ct);
                        queued.Completion.TrySetResult();
                    }
                    catch (OperationCanceledException) when (ct.IsCancellationRequested)
                    {
                        queued.Completion.TrySetCanceled(ct);
                    }
                    catch (Exception ex)
                    {
                        queued.Completion.TrySetException(ex);
                        if (!_manualDisconnect && IsTransientSendError(ex))
                        {
                            StartReconnectTimer();
                        }
                    }
    ...
    private async Task SendWithRetryAsync(object payload, CancellationToken ct)
    {
        Exception? lastError = null;
        for (var attempt = 1; attempt <= SendRetryAttempts; attempt++)
        {
            try
            {
                await WaitUntilReadyAsync(TimeSpan.FromMilliseconds(ReadyWaitTimeoutMs), ct);

                if (ConnectionMode == IMConnectionMode.NativeSDK && _nativeService != null)
                {
                    await SendViaNativeSDKAsync(payload, ct);
                }
                else
                {
                    await SendTextMessageAsync(payload, ct);
                }
                return;
            }
            catch (Exception ex) when (attempt < SendRetryAttempts && IsTransientSendError(ex))
            {
                lastError = ex;
                var delay = SendRetryBaseDelayMs * attempt;
                Logger.Warning(ex, "IM 消息发送失败（第 {Attempt}/{Max} 次），{Delay}ms 后重试", attempt, SendRetryAttempts, delay);
                await Task.Delay(delay, ct);
            }
        }

        throw lastError ?? new InvalidOperationException("IM 消息发送失败");
    }

    private static bool IsTransientSendError(Exception ex)
    {
    ...
    private async Task WaitUntilReadyAsync(TimeSpan timeout, CancellationToken ct)
    {
        var deadline = DateTime.UtcNow.Add(timeout);
        while (!_isReady && DateTime.UtcNow < deadline && !ct.IsCancellationRequested)
        {
        ...
        if (!_isReady)
        {
            throw new TimeoutException("IM 未就绪，等待发送超时");
        }
    }
    ...
    /// 通过原生 SDK 发送消息
    /// </summary>
    private async Task SendViaNativeSDKAsync(object payload, CancellationToken ct)
    {
        if (_nativeService == null || !_nativeService.IsLoggedIn)
        {
            throw new InvalidOperationException("原生 SDK 未就绪");
        }
        
        var payloadJson = EnsurePayloadToken(JsonSerializer.Serialize(payload));
        var toUserId = GetSendTargetUserId();
        ...
        if (!success)
        {
            throw new InvalidOperationException("原生 SDK 发送消息失败");
        }
    ...
    /// 发送文本消息
    /// </summary>
    private async Task SendTextMessageAsync(object payload, CancellationToken ct)
    {
        if (!_isReady || _ws?.State != WebSocketState.Open)
        {
            throw new InvalidOperationException("IM 未就绪（WebSocket 未连接）");
        }
        ...
        var sendRequest = new
        {
            ...
            ReqBody = new
            {
                ...
                To_Account = toUserId,
                MsgRandom = Random.Shared.Next(100000000, 999999999),
        ...
        await SendRawAsync(sendRequest, ct);
    }
        ...
        _heartbeatTask = Task.Run(async () =>
        {
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    await SendRawAsync(heartbeat, ct);
                    Logger.Debug("心跳已发送");
                }
                catch (OperationCanceledException) when (ct.IsCancellationRequested)
                {
    ...
    {
        if (_manualDisconnect || _disposed || Config?.AutoReconnect == false || Config == null)
        {
            return;
        }
        ...
        var delayMs = CalculateReconnectDelayMs(_reconnectAttempts);
        Logger.Information("安排 IM 自动重连: delay={Delay}ms, nextAttempt={Attempt}/{Max}", delayMs, _reconnectAttempts + 1, MaxReconnectAttempts);
    ...
    private static int CalculateReconnectDelayMs(int attemptsCompleted)
    {
        var exponent = Math.Min(attemptsCompleted, 5);
        var baseDelay = ReconnectBaseDelayMs * (1 << exponent);
        var capped = Math.Min(baseDelay, ReconnectMaxDelayMs);
        return capped + Random.Shared.Next(200, 900);
    }
    ...
    {
        if (_manualDisconnect || _disposed || Config == null || Status == DeviceStatus.Connected)
        {
            return;
        }
    ...
    /// <summary>
    /// 获取连接诊断快照（用于 UI/日志/故障排查）
    /// </summary>
    public YokonexIMConnectionDiagnostics GetConnectionDiagnostics()
    {
        return new YokonexIMConnectionDiagnostics
        {
            Mode = ConnectionMode,
            Status = Status,
            IsReady = _isReady,
            ...
            LastSendAtUtc = _lastSendAtUtc,
            LastReceiveAtUtc = _lastReceiveAtUtc
        };
    }

    private async Task CleanupTransportAsync(bool logoutNative)
    {
        StopHeartbeat();

        if (_nativeService != null)
        {
            await DetachNativeServiceAsync(logoutNative);
        }
        ...
        if (_ws != null)
        {
            try
            {
                ...
                {
                    await _ws.CloseAsync(WebSocketCloseStatus.NormalClosure, "Reconnect/Disconnect", CancellationToken.None);
                }
            }
            catch
            {
                // 忽略关闭阶段异常，后续统一 Dispose。
            }
    ...
    private async Task DetachNativeServiceAsync(bool logout)
    {
        if (_nativeService == null)
        {
            return;
        ...
        if (logout && _nativeService.IsLoggedIn)
        {
    ...
    private void ClearPendingSendQueue(Exception reason)
    {
        while (_sendQueue.TryDequeue(out var queued))
        {
            queued.Completion.TrySetException(reason);
        }
    }
    ...
    private void EnsureNotDisposed()
    {
        if (_disposed)
        {
            throw new ObjectDisposedException(nameof(YokonexIMAdapter));
        }
    }
    ...
    private sealed class QueuedMessage
    {
        ...
        public QueuedMessage(object payload)
        {
            Payload = payload;
    ...
    private bool TryResolveCredentials(ConnectionConfig config, out string uid, out string token, out string connectCode)
    {
        uid = string.Empty;
        token = string.Empty;
        connectCode = NormalizeConnectCode(config.ConnectCode);

        if (!string.IsNullOrWhiteSpace(connectCode) && TryParseConnectCode(connectCode, out uid, out token))
        {
            return true;
        }

        var legacyUid = config.UserId ?? config.Uid;
        if (string.IsNullOrWhiteSpace(legacyUid) || string.IsNullOrWhiteSpace(config.Token))
        {
            return false;
        }
        ...
        connectCode = NormalizeConnectCode($"{uid} {token}");
        Logger.Warning("检测到旧版 uid/token 配置，已在运行时转换为 connect_code。");
        return true;
    }

    private static bool TryParseConnectCode(string connectCode, out string uid, out string token)
    {
        uid = string.Empty;
        token = string.Empty;
        ...
        uid = segments[0].Trim();
        token = string.Join(" ", segments.Skip(1)).Trim();
        return !string.IsNullOrWhiteSpace(uid) && !string.IsNullOrWhiteSpace(token);
    }
    ...
    private string GetSendTargetUserId()
    {
        return NormalizeUid(_targetUserId) ?? _uid ?? throw new InvalidOperationException("IM 目标用户 ID 未初始化");
    }

    private string EnsurePayloadToken(string payloadJson)
    {
        ...
        if (payloadJson.Contains("\"token\"", StringComparison.OrdinalIgnoreCase))
        {
            return payloadJson;
        }

        var isSupportedCommand = payloadJson.Contains("\"code\":\"game_cmd\"", StringComparison.OrdinalIgnoreCase)
            || payloadJson.Contains("\"code\":\"game_info\"", StringComparison.OrdinalIgnoreCase);
        ...
        {
            Logger.Warning(ex, "为 IM 消息补齐 token 失败，使用原始 payload");
            return payloadJson;
        }
    }
    ...
    public void Dispose()
    {
        ...
        ClearPendingSendQueue(new ObjectDisposedException(nameof(YokonexIMAdapter)));

        _ws?.Dispose();
        _httpClient.Dispose();
        _sendQueueSignal.Dispose();
        _sendLock.Dispose();
...
/// IM 连接诊断信息
/// </summary>
public record YokonexIMConnectionDiagnostics
{
    public IMConnectionMode Mode { get; init; }
    public DeviceStatus Status { get; init; }
    public bool IsReady { get; init; }
    public bool IsLoggedIn { get; init; }
    public int QueueLength { get; init; }
    public int ReconnectAttempts { get; init; }
    public DateTime? LastSendAtUtc { get; init; }
    public DateTime? LastReceiveAtUtc { get; init; }
}
