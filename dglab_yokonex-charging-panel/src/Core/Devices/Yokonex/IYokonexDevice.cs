...
/// 统一 IM 命令下发能力。
/// </summary>
public interface IYokonexCommandDevice : IYokonexDevice
{
    /// <summary>
    /// 发送 game_cmd 事件
    /// </summary>
    Task SendGameCommandAsync(string eventId);

    /// <summary>
    /// 发送 game_info 数值事件
    /// </summary>
    Task SendGameInfoAsync(int data);
}
...
/// <summary>
/// 役次元智能锁扩展接口（预留）
/// 当前厂商协议尚未公开，先固化上层抽象，后续按文档补实现。
/// </summary>
public interface IYokonexSmartLockDevice : IYokonexDevice
{
    /// <summary>
    /// 查询锁状态
    /// </summary>
    Task<YokonexSmartLockState> QueryStateAsync();
    ...
    /// 临时解锁
    /// </summary>
    Task TemporaryUnlockAsync(int seconds);

    /// <summary>
    /// 锁状态变化事件
    /// </summary>
    event EventHandler<YokonexSmartLockState>? StateChanged;
}

/// <summary>
/// 智能锁状态（预留）
/// </summary>
public record YokonexSmartLockState
{
