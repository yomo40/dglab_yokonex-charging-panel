...
public enum GameEventType
{
    ...
    PressureChanged,    // 压力传感器变化
    EnemaStatusChanged, // 灌肠器工作状态上报
    DeviceBatteryChanged, // 设备电量上报
    ToyDeviceInfoChanged, // 跳蛋/飞机杯设备信息上报
    ChannelDisconnected,// 通道断开 (电极片脱落)
    ChannelConnected,   // 通道连接 (电极片接入)
    
    // DG-LAB 配件事件
    ExternalVoltageChanged, // 外部电压变化/越界触发（爪印配件）
...
public class EventBus : IDisposable
{
    ...
    public IDisposable Subscribe<T>(Action<T> handler) where T : class
    {
        ...
        {
            return _gameEvents.Subscribe(e => handler((T)(object)e));
        }
        if (typeof(T) == typeof(DeviceControlEvent))
        {
            return _deviceControlEvents.Subscribe(e => handler((T)(object)e));
        }
        if (typeof(T) == typeof(RemoteSyncEvent))
        {
            return _remoteSyncEvents.Subscribe(e => handler((T)(object)e));
        }
